...
func TestBitcoindEvents(t *testing.T) {
	...
	// Set up 2 ltcd miners.
	miner1, miner2 := setupMiners(t)

	for _, test := range tests {
		test := test
		t.Run(test.name, func(t *testing.T) {
			// Set up a litecoind node and connect it to miner 1.
...
func testNotifications(t *testing.T, miner *rpctest.Harness,
	...
	tx, err := miner.CreateTransaction(
		[]*wire.TxOut{{Value: 10000, PkScript: script}}, 5, false,
	)
	require.NoError(t, err)
...
// setupBitcoind starts up a litecoind node with either a zmq connection or
...
func setupBitcoind(t *testing.T, minerAddr string,
	...
	tempBitcoindDir, err := os.MkdirTemp("", "litecoind")
	require.NoError(t, err)
...
func randPubKeyHashScript() ([]byte, *btcec.PrivateKey, error) {
	...
	addrScript, err := ltcutil.NewAddressPubKeyHash(
